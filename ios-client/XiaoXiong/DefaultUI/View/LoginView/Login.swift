...
//                        .clipShape(Circle())
    ...
    func getConfig()  async {
        ...
        print("LoginView 请求配置接口: \(configUrl)")
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async {
                ...
                if let jsonString = String(data: data, encoding: .utf8) {
                    print("LoginView Response data: \(jsonString)")
                }
                ...
                if let jsonResponse = try? JSONDecoder().decode(ConfigResponse.self, from: data){
                    if jsonResponse.status == "success" {
                        ...
                        if let configData = jsonResponse.data {
                            ...
                            print("LoginView 配置获取成功: is_captcha=\(configData.isCaptcha ?? 0)")
                        }
                    } else {
                        print("LoginView 配置获取失败: \(jsonResponse.message ?? "未知错误")")
                    }
                } else {
                    print("LoginView 配置解析失败")
                }
            }
        }
